...
class SpheroEduAPI:
    ...
    async def __background(self):
        while True:
            try:
                if await asyncio.wait_for(self.__stopped.wait(), 0.8):
                    break
    ...
    def set_speed(self, speed: int):
        ...
        Ollie is almost three times faster than Sphero. For example, use ``set_speed(188)`` to set the speed to
        188 which persists until you set a different speed. You can also read the real-time velocity value in
        centimeters per second reported by the motor encoders.
        """
        if isinstance(self.__toy, Mini) and speed != 0:
            speed = round((speed + 126) * 2 / 3) if speed > 0 else round((speed - 126) * 2 / 3)
        self.__speed = bound_value(-255, speed, 255)
    ...
    async def spin(self, angle: int, duration: float):
        ...
        For example, to spin the robot 360° over 1s, use: ``await spin(360, 1)``.
    ...
    async def raw_motor(self, left: int, right: int, duration: float):
        ...
        to full power for 4s, making the robot jump off the ground, use ``await raw_motor(255, 255, 4)``."""
    ...
    def reset_aim(self):
        ...
        asyncio.ensure_future(ToyUtil.reset_heading(self.__toy))

    # Star Wars Droid Movements
    async def play_animation(self, animation: IntEnum):  # FIXME check consistency
    ...
    async def set_stance(self, stance: Stance):
        """Changes the stance between bipod and tripod. Set to bipod using ``await set_stance(Stance.Bipod)`` and
        to tripod using ``await set_stance(Stance.Tripod)``. Tripod is required for rolling."""
        if isinstance(self.__toy, (R2D2, R2Q5)):
    ...
    async def set_waddle(self, waddle: bool):
        """Turns the waddle walk on using ``await set_waddle(true)`` and off using ``await set_waddle(false)``."""
        if isinstance(self.__toy, (R2D2, R2Q5)):
            async with self.__updating:
                await self.__stop_all()
    ...
    def set_front_led(self, color: Color):
        ...
        if isinstance(self.__toy, (R2D2, R2Q5, BOLT, RVR)):
            ...
            asyncio.ensure_future(ToyUtil.set_front_led(self.__toy, **self.__leds['front']._asdict()))
    ...
    def set_back_led(self, color: Union[Color, int]):
        ...
        Use :func:`asyncio.sleep` to set it on for a duration. For example, to create a dim and a bright blink
        sequence use::
        ...
        if isinstance(color, int):
            ...
            asyncio.ensure_future(ToyUtil.set_back_led_brightness(self.__toy, self.__leds['back'].b))
        elif isinstance(self.__toy, (R2D2, R2Q5, BOLT, RVR)):
            self.__leds['back'] = bound_color(color, self.__leds['back'])
            asyncio.ensure_future(ToyUtil.set_back_led(self.__toy, **self.__leds['back']._asdict()))
    ...
    async def fade(self, from_color: Color, to_color: Color, duration: float):
        ...
        green to blue over 3s, use: ``await fade(Color(0, 255, 0), Color(0, 0, 255), 3.0)``."""
    ...
    async def strobe(self, color: Color, period: float, count: int):
        ...
        ``await strobe(Color(255, 57, 66), 3 / 15 / 2, 15)``."""
    ...
    async def _collision_detected_notify(self, args: CollisionDetectedArgs):
